...
#from app.database.requests import user_pk
...
async def cmd_start(message: Message):
    await rq.set_user(message.from_user.id), await message.reply('ЭТАП 1 ПРОДЕН!')
...
async def randomizer1(message: Message):
    ...
    match text:
        case '1': await message.reply(f'{rand}')
        ...
        case 'урон': await message.reply(f'{rand1_100}')
        case 'кубик': await message.reply_dice()
        case 'ахуеть': await message.reply('<b>Звуки бравл старса</b>',parse_mode='HTML')
#        case 'рп профиль': await rq.user_number(message.from_user.id), await message.reply(user_pk)
